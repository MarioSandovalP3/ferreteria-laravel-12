<div class="w-full max-w-9xl mx-auto">
    @if($showForm)
        @include('livewire.expenses.form')
    @else
        {{-- Header Section --}}
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
                ...
                <flux:subheading size="lg" class="text-zinc-500 dark:text-zinc-400 mt-1">
                    {{ __('common.manage_operational_expenses') }}
                </flux:subheading>
            </div>
                ...
                <div class="flex flex-col sm:flex-row gap-4 justify-between">
                    ...
                    <div class="flex gap-3">
                        <flux:select 
                            ...
                            class="w-48"
                        >
                            <flux:select.option value="">{{ __('common.all_categories') }}</flux:select.option>
                    ...
                    <tbody class="bg-white dark:bg-zinc-900 divide-y divide-zinc-200 dark:border-zinc-800">
                        @forelse($expenses as $expense)
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-zinc-600 dark:text-zinc-400">
                                    {{ __('common.' . $expense->payment_method) }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end gap-3">
                                        ...
                                        <button wire:click="destroy({{ $expense->id }})" 
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-sm font-medium text-zinc-900 dark:text-zinc-100">{{ __('common.no_expenses_found') }}</p>
                                        <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-1">{{ __('common.get_started_by_creating_new_expense') }}</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
